...
@Suite struct KeysAndRoomsTests {
    ...
    @Test func test_0() async {
        ...
        let p_rooms = InputParser.parse2DIntArray(params[0])
        guard p_rooms.count <= 1000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: rooms 2D array too large (\(p_rooms.count))")
            return
        }

        let solution = Solution()
        let result = solution.canVisitAllRooms(p_rooms)
