...
enum LC_valid_sudoku {
    ...
    @Suite struct ValidSudokuTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_board.count == 9 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: board[i].length == 9")
                return
            }
