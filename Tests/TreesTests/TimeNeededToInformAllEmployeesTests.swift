...
@Suite struct TimeNeededToInformAllEmployeesTests {
    ...
    @Test func test_0() async {
        ...
        let p_headID = InputParser.parseInt(params[1])
        let p_manager = InputParser.parseIntArray(params[2])
        guard p_manager.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: manager array too large (\(p_manager.count))")
            return
        }
        let p_informTime = InputParser.parseIntArray(params[3])
        guard p_informTime.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: informTime array too large (\(p_informTime.count))")
            return
        }
