...
@Suite struct ConstructTheRectangleTests {
    ...
    @Test func test_0() async {
        ...
        let p_area = InputParser.parseInt(params[0])

        // Constraint precondition checks
        guard p_area >= 1 && p_area <= 107 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= area <= 107")
            return
        }
