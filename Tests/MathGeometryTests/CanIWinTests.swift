...
@Suite struct CanIWinTests {
    ...
    @Test func test_0() async {
        ...
        let p_maxChoosableInteger = InputParser.parseInt(params[0])
        let p_desiredTotal = InputParser.parseInt(params[1])

        // Constraint precondition checks
        guard p_maxChoosableInteger >= 1 && p_maxChoosableInteger <= 20 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= maxChoosableInteger <= 20")
            return
        }
        guard p_desiredTotal >= 0 && p_desiredTotal <= 300 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= desiredTotal <= 300")
            return
        }
