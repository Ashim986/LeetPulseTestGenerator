...
@Suite struct MaximumProductOfThreeNumbersTests {
    ...
    @Test func test_0() async {
        ...
        guard p_nums.count >= 3 && p_nums.count <= 104 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 3 <= nums.length <= 104")
            return
        }
