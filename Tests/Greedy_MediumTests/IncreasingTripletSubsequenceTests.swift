...
enum LC_increasing_triplet_subsequence {
    ...
    @Suite struct IncreasingTripletSubsequenceTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_nums.count >= 1 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= nums.length <= 5 * 105")
                return
            }
