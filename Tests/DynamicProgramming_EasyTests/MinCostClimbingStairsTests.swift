...
enum LC_min_cost_climbing_stairs {
    ...
    @Suite struct MinCostClimbingStairsTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_cost.count >= 2 && p_cost.count <= 1000 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 2 <= cost.length <= 1000")
                return
            }
            guard p_cost.allSatisfy({ $0 >= 0 && $0 <= 999 }) else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= cost[i] <= 999")
                return
            }
