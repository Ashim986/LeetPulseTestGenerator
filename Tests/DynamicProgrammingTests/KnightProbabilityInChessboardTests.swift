...
@Suite struct KnightProbabilityInChessboardTests {
    ...
    @Test func test_0() async {
        ...
        let p_row = InputParser.parseInt(params[2])
        let p_column = InputParser.parseInt(params[3])

        // Constraint precondition checks
        guard p_n >= 1 && p_n <= 25 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= n <= 25")
            return
        }
        guard p_k >= 0 && p_k <= 100 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= k <= 100")
            return
        }
