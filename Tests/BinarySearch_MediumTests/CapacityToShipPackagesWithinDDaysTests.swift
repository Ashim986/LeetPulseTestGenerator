...
enum LC_capacity_to_ship_packages_within_d_days {
    ...
    @Suite struct CapacityToShipPackagesWithinDDaysTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_weights.count >= 1 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= days <= weights.length <= 5 * 104")
                return
            }
            guard p_weights.allSatisfy({ $0 >= 1 && $0 <= 500 }) else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= weights[i] <= 500")
                return
            }
