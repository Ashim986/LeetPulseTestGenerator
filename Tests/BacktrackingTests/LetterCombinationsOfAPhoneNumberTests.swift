...
@Suite struct LetterCombinationsOfAPhoneNumberTests {
    ...
    @Test func test_0() async {
        ...
        guard p_digits.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: digits string too long (\(p_digits.count))")
            return
        }

        // Constraint precondition checks
        guard p_digits.count >= 0 && p_digits.count <= 4 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= digits.length <= 4")
            return
        }

        let solution = Solution()
        let result = solution.letterCombinations(p_digits)
