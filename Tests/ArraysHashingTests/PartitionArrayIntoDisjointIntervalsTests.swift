...
@Suite struct PartitionArrayIntoDisjointIntervalsTests {
    ...
    @Test func test_0() async {
        ...
        guard p_nums.allSatisfy { $0 >= 0 && $0 <= 106 } else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= nums[i] <= 106")
            return
        }
