...
@Suite struct SubdomainVisitCountTests {
    ...
    @Test func test_0() async {
        ...
        let p_cpdomains = InputParser.parseStringArray(params[0])
        guard p_cpdomains.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: cpdomains array too large (\(p_cpdomains.count))")
            return
        }
