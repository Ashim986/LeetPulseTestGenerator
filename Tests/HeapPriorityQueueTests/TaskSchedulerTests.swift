...
private class Solution {
    func leastInterval(_ tasks: [Character], _ n: Int, trace: Trace) -> Int {
        var freq = [Int](repeating: 0, count: 26)
        for task in tasks {
            freq[Int(task.asciiValue!) - 65] += 1
        }
        let nonZeroFreqs = freq.filter { $0 > 0 }
        trace.step("count frequencies", ["taskCount": tasks.count, "cooldown": n, "frequencies": nonZeroFreqs])

        let maxFreq = freq.max()!
        let numMaxTasks = freq.filter { $0 == maxFreq }.count
        trace.step("find max freq", ["maxFreq": maxFreq, "numMaxTasks": numMaxTasks])

        let minTime = (maxFreq - 1) * (n + 1) + numMaxTasks
        let result = max(minTime, tasks.count)
        trace.step("compute result", ["formulaResult": minTime, "taskCount": tasks.count, "result": result])
        return result
    }
}
...
@Suite struct TaskSchedulerTests {
    ...
    @Test func test_0() async {
        ...
        let p_tasks = InputParser.parseCharacterArray(params[0])
        guard p_tasks.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: tasks array too large (\(p_tasks.count))")
            return
        }
        ...
        guard p_tasks.count >= 1 && p_tasks.count <= 104 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= tasks.length <= 104")
            return
        }
        guard p_n >= 0 && p_n <= 100 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 0 <= n <= 100")
            return
        }

        let solution = Solution()
        let trace = Trace()
        let result = solution.leastInterval(p_tasks, p_n, trace: trace)
