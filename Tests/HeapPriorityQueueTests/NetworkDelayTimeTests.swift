...
private class Solution {
    func networkDelayTime(_ times: [[Int]], _ n: Int, _ k: Int, trace: Trace) -> Int {
        var graph = [Int: [(Int, Int)]]()
        for t in times {
            graph[t[0], default: []].append((t[1], t[2]))
        }
        trace.step("build graph", ["n": n, "source": k, "edgeCount": times.count])
        ...
        while !heap.isEmpty {
            ...
            dist[node] = d
            trace.step("visit node", ["node": node, "dist": d, "visited": dist.count, "heapSize": heap.count])
        ...
        if dist.count != n {
            trace.step("unreachable", ["visitedCount": dist.count, "totalNodes": n])
            return -1
        }
        let result = dist.values.max()!
        trace.step("result", ["maxDelay": result, "allDists": dist.count])
        return result
    }
}
...
@Suite struct NetworkDelayTimeTests {
    ...
    @Test func test_0() async {
        ...
        let p_times = InputParser.parse2DIntArray(params[0])
        guard p_times.count <= 1000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: times 2D array too large (\(p_times.count))")
            return
        }
        ...
        guard p_times.count >= 1 && p_times.count <= 6000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= times.length <= 6000")
            return
        }

        let solution = Solution()
        let trace = Trace()
        let result = solution.networkDelayTime(p_times, p_n, p_k, trace: trace)
