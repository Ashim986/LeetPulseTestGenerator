...
@Suite struct MinimumSizeSubarraySumTests {
    ...
    @Test func test_0() async {
        ...
        let p_nums = InputParser.parseIntArray(params[1])
        ...
        guard p_target >= 1 && p_target <= 109 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= target <= 109")
            return
        }
