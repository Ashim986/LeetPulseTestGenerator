...
enum LC_unique_morse_code_words {
    ...
    @Suite struct UniqueMorseCodeWordsTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_words.allSatisfy({ $0.count >= 1 && $0.count <= 12 }) else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= words[i].length <= 12")
                return
            }
