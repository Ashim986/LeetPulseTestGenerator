...
enum LC_verifying_an_alien_dictionary {
    ...
    @Suite struct VerifyingAnAlienDictionaryTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_order.count <= 100_000 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: order string too long (\(p_order.count))")
                return
            }
            ...
            guard p_order.count == 26 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: order.length == 26")
                return
            }
