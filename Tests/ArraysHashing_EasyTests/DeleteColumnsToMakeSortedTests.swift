...
enum LC_delete_columns_to_make_sorted {
    ...
    @Suite struct DeleteColumnsToMakeSortedTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_strs.allSatisfy({ $0.count >= 1 && $0.count <= 1000 }) else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= strs[i].length <= 1000")
                return
            }
