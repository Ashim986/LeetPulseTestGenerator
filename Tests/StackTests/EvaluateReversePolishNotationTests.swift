...
@Suite struct EvaluateReversePolishNotationTests {
    ...
    @Test func test_0() async {
        ...
        let p_tokens = InputParser.parseStringArray(params[0])
        ...
        guard p_tokens.count >= 1 && p_tokens.count <= 104 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= tokens.length <= 104")
            return
        }
