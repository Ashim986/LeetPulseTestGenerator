...
@Suite struct CarFleetTests {
    ...
    @Test func test_0() async {
        ...
        let p_target = InputParser.parseInt(params[0])
        let p_position = InputParser.parseIntArray(params[1])
        guard p_position.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: position array too large (\(p_position.count))")
            return
        }
        let p_speed = InputParser.parseIntArray(params[2])
        guard p_speed.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: speed array too large (\(p_speed.count))")
            return
        }
