...
@Suite struct MostProfitAssigningWorkTests {
    ...
    @Test func test_0() async {
        ...
        let p_difficulty = InputParser.parseIntArray(params[0])
        guard p_difficulty.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: difficulty array too large (\(p_difficulty.count))")
            return
        }
        let p_profit = InputParser.parseIntArray(params[1])
        guard p_profit.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: profit array too large (\(p_profit.count))")
            return
        }
        let p_worker = InputParser.parseIntArray(params[2])
        guard p_worker.count <= 100_000 else {
            await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, outputMatches: false, orderMatters: orderMatters, errorMessage: "Constraint violation: worker array too large (\(p_worker.count))")
            return
        }
