...
enum LC_cheapest_flights_within_k_stops {
    ...
    @Suite struct CheapestFlightsWithinKStopsTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_flights.count == 3 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: flights[i].length == 3")
                return
            }
