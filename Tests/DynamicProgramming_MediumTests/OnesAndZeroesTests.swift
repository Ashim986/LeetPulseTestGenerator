    ...
    @Suite struct OnesAndZeroesTests {
        ...
        @Test static func test_0() async {
            ...
            guard p_strs.count >= 1 && p_strs.count <= 600 else {
                await ResultRecorderActor.shared.record(slug: slug, topic: topic, testId: testId, input: rawInput, originalExpected: expectedOutput, computedOutput: "", isValid: false, status: "parse_error", orderMatters: orderMatters, errorMessage: "Constraint violation: 1 <= strs.length <= 600")
                return
            }
